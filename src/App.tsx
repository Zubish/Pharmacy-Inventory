...
function compactNumber(value: number) {
  ...
  const truncate = (amount: number, divisor: number) =>
    (Math.trunc((amount / divisor) * 100) / 100).toFixed(2);
...
function sellableUnitLabel(medicine: Medicine) {
  ...
  return packSize > 1
    ? `${packSize} ${unit} per ${medicine.unit || "pack"}`
    : unit;
}
...
function medicineUnitCostFromContainerCost(
  ...
  return cost > 0
    ? cost / medicineUnitsPerContainer(medicine)
...
function pricingPolicy(settings: AppSettings) {
  const roundingRule = pricingRoundingRules.includes(
    settings.pricingRoundingRule ?? 10,
  )
    ? (settings.pricingRoundingRule ?? 10)
    : 10;
  return {
    ...
    roundingRule,
    categoryMarkupPercentages: normalizeMarkupMap(
      settings.categoryMarkupPercentages,
    ),
    ...
    unusualMarkupPercent: Math.max(
      0,
      Number(settings.unusualMarkupPercent ?? 80) || 0,
    ),
    costChangeWarningPercent: Math.max(
      0,
      Number(settings.costChangeWarningPercent ?? 30) || 0,
    ),
  };
}
...
function productSpecificKeys(
  ...
  return [`${itemType}:${product.id}`, product.id, product.sku, product.name]
    .map((key) => key.trim().toLowerCase())
    .filter(Boolean);
}
...
function markupForItem(
  ...
  for (const key of productSpecificKeys(itemType, item)) {
    ...
    if (value !== undefined)
      return { percent: value, source: "Product override" };
  }
  const category = item.category.trim().toLowerCase();
  if (category && policy.categoryMarkupPercentages[category] !== undefined) {
    return {
      percent: policy.categoryMarkupPercentages[category],
      source: "Category markup",
    };
  }
  return { percent: policy.globalMarkupPercent, source: "Global markup" };
}

function calculatedSellingPrice(
  db: Database,
  itemType: "medicine" | "product",
  itemId: string,
  unitCost: number,
  ...
  return Object.fromEntries(
    value
      ...
      .map((line) => {
        ...
        return [
          key.trim().toLowerCase(),
          Math.max(0, Number(amount.trim()) || 0),
        ] as const;
      })
      .filter(([key]) => key),
  );
...
function dosageFormRulesToText(map?: Record<string, string>) {
  return Object.entries(
    Object.keys(map ?? {}).length ? (map ?? {}) : defaultDosageFormLabelRules,
  )
  ...
  return Object.fromEntries(
    value
      ...
      .map((line) => {
        ...
        return [
          key.trim().toLowerCase(),
          instructionParts.join(separator).trim(),
        ] as const;
      })
...
function medicineLabelRulesToText(rules?: MedicineLabelRule[]) {
  ...
  return source
    .map(
      (rule) =>
        `${rule.match} | ${rule.label} | ${rule.instruction}${rule.followUpMessage ? ` | ${rule.followUpMessage}` : ""}`,
    )
    .join("\n");
}
...
function textToMedicineLabelRules(value: string): MedicineLabelRule[] {
  return value
    ...
    .filter(Boolean)
    .map((line, index) => {
      const [match = "", label = "", instruction = "", followUpMessage = ""] =
  ...
  return {
    ...
    settings: {
      ...
      defaultFollowUpMessage:
        "Please contact the clinic pharmacy if symptoms persist, your condition worsens, or you notice unusual side effects.",
...
function getStockRows(db: Database): StockRow[] {
  return db.batches
    .map((batch) => {
      ...
      return {
        ...
        status:
          days < 0
            ? "expired"
            : days <= db.settings.nearExpiryDays
              ? "near-expiry"
              : "ok",
      } satisfies StockRow;
    })
    ...
    .sort(
      (a, b) =>
        a.medicine.brandName.localeCompare(b.medicine.brandName) ||
...
function aggregateMedicineStock(rows: StockRow[]) {
  ...
  rows.forEach((row) => {
    totals.set(
      row.medicine.id,
      (totals.get(row.medicine.id) ?? 0) + row.quantity,
    );
  });
  return totals;
}
...
function getBranchName(db: Database, branchId: string) {
  return (
    db.branches.find((branch) => branch.id === branchId)?.name ??
...
function patientDisplayName(sale: Sale) {
  return (
    ...
    (sale.customerPhone.trim() ? sale.customerPhone.trim() : "Walk-in patient")
  );
}
...
function getSaleItemLabel(db: Database, item: Sale["items"][number]) {
  ...
  if (item.itemType === "product")
    return (
      db.products.find((product) => product.id === item.productId)?.name ||
      "Retail item"
    );
  return (
    db.medicines.find((medicine) => medicine.id === item.medicineId)
      ?.brandName || "Medicine"
  );
}
...
const builtInMedicineLabelRules: MedicineLabelRule[] = [
  ...
  },
  {
    ...
    followUpMessage:
      "Please avoid alcohol while taking metronidazole and for at least 48 hours after completing the course. Contact the clinic pharmacy if you feel unwell.",
    enabled: true,
  },
  {
    ...
    instruction:
      "Take exactly as prescribed and complete the full course. Do not skip doses.",
    followUpMessage:
      "Please complete the antibiotic course exactly as directed, even if you feel better. Contact the clinic pharmacy if you develop rash, severe diarrhea, swelling, or breathing difficulty.",
    enabled: true,
  },
  {
    id: "built-in-aprovel",
  ...
  },
  {
    ...
    followUpMessage:
      "Please take Natrilix in the morning unless otherwise directed, and contact the clinic pharmacy if you feel dizzy or unusually weak.",
    enabled: true,
  },
  {
    ...
    instruction:
      "Take with meals unless otherwise directed. Do not skip meals.",
    followUpMessage:
      "Remember to take your diabetes medicine with meals unless your prescriber advised differently. Keep monitoring your blood sugar and contact the clinic pharmacy if you feel unwell.",
    enabled: true,
  },
  {
    id: "built-in-bp",
    ...
    instruction:
      "Take at the same time daily. Do not stop suddenly unless your prescriber advises.",
    followUpMessage:
      "Please take your blood pressure medicine at the same time daily and keep monitoring your readings. Contact the clinic pharmacy if you notice severe dizziness or swelling.",
    enabled: true,
  },
  {
    ...
    label: "Dose limit",
    instruction:
      "Take only as directed. Do not exceed the recommended daily dose or combine with other paracetamol-containing products.",
    followUpMessage:
      "Use paracetamol only as directed and avoid taking it with other products that also contain paracetamol.",
    enabled: true,
  },
];
...
function splitRuleKeys(value: string) {
  return value
    .split(",")
    .map((item) => normalizeCounselingText(item))
    .filter(Boolean);
}

function dosageFormRules(settings: AppSettings) {
  return Object.keys(settings.dosageFormLabelRules ?? {}).length
    ? (settings.dosageFormLabelRules ?? {})
    : defaultDosageFormLabelRules;
}

function medicineRules(settings: AppSettings) {
  return [
    ...(settings.medicineLabelRules ?? []),
    ...builtInMedicineLabelRules,
  ].filter(
    (rule) =>
      rule.enabled !== false && rule.match.trim() && rule.instruction.trim(),
  );
}

function medicineInstructionSource(medicine: Medicine) {
  return normalizeCounselingText(
    `${medicine.brandName} ${medicine.genericName} ${medicine.category} ${medicine.form}`,
  );
}
...
function dosageFormSuggestion(
  settings: AppSettings,
  medicine: Medicine,
): CounselingSuggestion | undefined {
  ...
  const label =
    splitRuleKeys(keys)[0]?.replace(/\b\w/g, (char) => char.toUpperCase()) ||
    "Dosage form";
  return { id: `form-${keys}`, label, instruction };
}

function counselingSuggestionsForMedicine(
  db: Database,
  medicine: Medicine,
): CounselingSuggestion[] {
  const suggestions = [
  ...
  ].filter((suggestion): suggestion is CounselingSuggestion =>
    Boolean(suggestion),
  );
...
function combinedInstruction(
  ...
  const instructions = suggestions
    .map((suggestion) => suggestion.instruction.trim())
    .filter(Boolean);
  return Array.from(new Set(instructions)).join(" ") || fallback;
}

function defaultLabelInstruction(
  db: Database,
  itemType: "medicine" | "product",
  itemId: string,
) {
  ...
  if (!medicine) return "";
  const medicineName = [medicine.brandName, medicine.strength]
    .filter(Boolean)
    .join(" ");
  return combinedInstruction(
    counselingSuggestionsForMedicine(db, medicine),
    `Take ${medicineName} exactly as prescribed. Contact the clinic pharmacy if you notice unusual side effects.`,
  );
}

function medicineFollowUpInstruction(medicine: Medicine) {
  const followUp = medicineRules({ ...createEmptyDatabase().settings }).find(
    (rule) => ruleMatchesMedicine(rule, medicine),
  )?.followUpMessage;
  return (
    followUp ||
    `Take ${medicine.brandName} exactly as prescribed and contact the clinic pharmacy if you notice unusual side effects.`
  );
}

function buildDispensingFollowUpMessage(
  db: Database,
  items: Array<{ itemType: "medicine" | "product"; itemId: string }>,
) {
  const medicineMessages = items
    ...
    .flatMap((medicine) => {
      const specific = counselingSuggestionsForMedicine(db, medicine)
        .map((suggestion) => suggestion.followUpMessage)
        .filter((message): message is string => Boolean(message?.trim()));
      return specific.length
        ? specific
        : [medicineFollowUpInstruction(medicine)];
    });
...
function buildRefillRows(db: Database) {
    ...
    profile.sales.forEach((sale) => {
      sale.items.forEach((item) => {
        if (
          ...
          return;
        const refillDueAt =
          item.refillDueAt || addDaysToDate(sale.soldAt, item.daysSupply);
  ...
  return [...latestByPatientMedicine.values()].sort((a, b) =>
    a.refillDueAt.localeCompare(b.refillDueAt),
  );
}

function patientCareMessage(
  pharmacyName: string,
  patientName: string,
  body: string,
) {
...
function refillMessage(
  pharmacyName: string,
  row: ReturnType<typeof buildRefillRows>[number],
) {
...
function saleFollowUpBody(db: Database, sale: Sale) {
  ...
  const legacyNotes = sale.items
    .filter(
      (item) =>
        item.itemType === "medicine" &&
        (item.followUpMessage || item.counselingNote),
    )
    .map(
      (item) =>
        `${getSaleItemLabel(db, item)}: ${item.followUpMessage || item.counselingNote}`,
    );
...
function getPrimaryAdminId(db: Database) {
  return (
    ...
    db.users.find((user) => user.role === "admin" && user.status === "active")
      ?.id ||
    ""
  );
}
...
function isSuperAdmin(db: Database, user: User | null | undefined) {
  return Boolean(
    user && user.role === "admin" && user.id === getPrimaryAdminId(db),
  );
}
...
function canManagePendingMedication(
  db: Database,
  user: User,
  branchId: string,
) {
...
function getUserHomeBranch(db: Database, user: User | null | undefined) {
  ...
  const activeManagedBranchIds = user.managedBranchIds.filter((branchId) =>
    hasActiveBranchAssignment(user, branchId),
  );
  const activeBranchIds = user.branchIds.filter((branchId) =>
    hasActiveBranchAssignment(user, branchId),
  );
  const homeId = activeManagedBranchIds[0] || activeBranchIds[0];
  return (
    activeBranches.find((branch) => branch.id === homeId) ??
    activeBranches[0] ??
    db.branches[0]
  );
}

function prioritizeAssignedBranch(
  branches: Branch[],
...
function getOtherAssignedBranch(db: Database, user: User, branchId: string) {
  ...
  return otherId
    ? db.branches.find((branch) => branch.id === otherId)
    : undefined;
}
...
function findReceivableItem(
  db: Database,
  query: string,
): { itemType: "medicine"; itemId: string; label: string } | undefined {
  ...
  const medicineMatches = db.medicines
    ...
    .map((medicine) => ({
      ...
      exact: [medicine.sku, medicine.nafdacNumber, ...medicine.barcodes].some(
  ...
  return medicineMatches
    ...
    .sort(
      (a, b) =>
        Number(b.exact) - Number(a.exact) || a.label.localeCompare(b.label),
    )[0];
}
...
function isNotificationVisible(
  ...
  if (notification.audience === "super-admin")
    return isSuperAdmin(db, currentUser);
  if (!notification.branchId) return true;
  if (
    notification.requiredPermission === "manage-branch" &&
    !canManageBranch(db, currentUser, notification.branchId)
  )
    return false;
...
function getQuestSteps(db: Database, currentUser: User): QuestStep[] {
  ...
  return [...baseSteps, ...roleSteps].filter((step) => {
    ...
    if (step.roles && !step.roles.includes(currentUser.role) && !superAdmin)
      return false;
    return true;
  });
}
...
function buildNotifications(
  ...
  const incomingRequisitions = relevantRequisitions.filter(
    (request) => request.status === "pending",
  );
  const releasedRequisitions = relevantRequisitions.filter(
    (request) =>
      request.status === "released" &&
      canViewBranch(db, currentUser, request.requestingBranchId),
  );
  const handledRequisitions = relevantRequisitions.filter(
    (request) =>
      request.requesterUserId === currentUser.id &&
      request.status !== "pending",
  );
  const branchAccessRequests = db.branchAccessRequests.filter(
    (request) =>
      request.status === "pending" &&
  ...
  if (isSuperAdmin(db, currentUser)) {
    pendingUsers.forEach((user) => {
      notifications.push({
        ...
        detail:
          "Admin should assign the correct role and activate the account.",
        view: "users",
  ...
  incomingRequisitions.forEach((request) => {
    ...
    notifications.push({
      ...
      tone: "info",
      title: viewingSourceBranch
        ? `Medicine request from ${requestingBranch}`
        : `Medicine request to ${sourceBranch}`,
      ...
      view: "medicines",
      branchId:
        activeBranch?.id === request.requestingBranchId
  ...
  handledRequisitions.forEach((request) => {
    notifications.push({
      ...
      tone:
        request.status === "received" || request.status === "fulfilled"
          ? "good"
          : request.status === "released"
            ? "info"
            : "warning",
...
function App() {
  ...
  const branchMenuBranches = useMemo(
    () => prioritizeAssignedBranch(activeBranches, assignedBranch?.id),
    [activeBranches, assignedBranch?.id],
  );
  const activeBranch =
  ...
  const activeBranchStockRows = useMemo(
    () =>
      activeBranch
        ? stockRows.filter((row) => row.batch.branchId === activeBranch.id)
        : stockRows,
    [activeBranch, stockRows],
  );
  const permittedStockRows = useMemo(
    () =>
      currentUser
        ? stockRows.filter((row) =>
            canViewBranch(db, currentUser, row.batch.branchId),
          )
        : [],
    [currentUser, db, stockRows],
  );
  ...
  const dashboardStockTotals = useMemo(
    () => aggregateMedicineStock(dashboardStockRows),
    [dashboardStockRows],
  );
  const medicinePageStockRows = activeBranchStockRows;
  const medicinePageStockTotals = useMemo(
    () => aggregateMedicineStock(medicinePageStockRows),
    [medicinePageStockRows],
  );
  const notificationStockRows = activeBranch
    ? activeBranchStockRows
    : permittedStockRows;
  const notificationStockTotals = useMemo(
    () => aggregateMedicineStock(notificationStockRows),
    [notificationStockRows],
  );
  const canWriteActiveBranch =
    currentUser && activeBranch
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!boot.settings) {
          throw new Error(
            "Backend API is not available. Run npx vercel dev with DATABASE_URL for API-backed flows.",
          );
        }
        ...
        if (getStoredToken()) {
          ...
          setActiveBranchId(
            getUserHomeBranch(state.db, state.currentUser)?.id ??
              state.db.branches.find((branch) => branch.active)?.id ??
              "main",
          );
        }
      } catch (error) {
        ...
        setConnectionError(
          error instanceof Error
            ? error.message
            : "Unable to connect to the pharmacy backend",
        );
      } finally {
        setLoading(false);
      }
  ...
  const forgetBrowserUser = useCallback(
    (userId?: string | null) => {
      ...
      if (!userId) return;
      const workspaceKey =
        db.settings.companySlug || db.settings.companyCode || "file";
  ...
  async function createFirstAdmin(input: SetupInput) {
    ...
    try {
      ...
      setActiveBranchId(
        getUserHomeBranch(result.db, result.currentUser)?.id ??
  ...
  async function triggerSecurityPanic() {
    ...
    if (
      !window.confirm(
        "Secure this account now? Every active session, including this browser, will be signed out and remembered browsers will be cleared.",
      )
    )
      return;
    try {
  ...
  function clampDrawerHandleTop(value: number) {
    ...
    const maximum =
      typeof window === "undefined"
        ...
        : Math.max(minimum, window.innerHeight - 76);
  ...
  function moveDrawerHandle(event: ReactPointerEvent<HTMLButtonElement>) {
    ...
    const nextTop =
      drawerDragRef.current.startTop +
      event.clientY -
      drawerDragRef.current.startY;
    ...
    >
      ...
      <aside className="sidebar">
          ...
          {views.map(({ id: viewId, label, icon: Icon, adminOnly }) => {
            ...
            return (
              ...
              >
                <Icon size={18} />
                <span>{label}</span>
                {viewId === "users" && pendingAdminTasks > 0 && (
                  <b className="nav-badge">{pendingAdminTasks}</b>
                )}
                {viewId === "chat" && unreadChat > 0 && (
                  <b className="nav-badge">{unreadChat}</b>
                )}
        ...
        <div className="sidebar-bottom">
          <button
            ...
            type="button"
            onClick={triggerSecurityPanic}
            title="Sign out other sessions and record a security alert"
          >
            <ShieldCheck size={17} />
      ...
      >
        {sidebarCollapsed ? (
          ...
          <ChevronLeft size={30} strokeWidth={3.2} />
        )}
      </button>
      ...
      <main className="workspace">
                ...
                >
                  ...
                  <span className="branch-trigger-text">
                    ...
                    <small>
                      {canAdmin
                        ? "Global super admin"
                        : assignedBranch?.id === activeBranch.id
                          ? "Assigned branch"
                ...
                {branchMenuOpen && (
                  <div className="branch-menu">
                    {branchMenuBranches.map((branch) => (
                      <button
                        className={
                          branch.id === activeBranch.id ? "active" : ""
                        }
                        key={branch.id}
                        type="button"
                      ...
                      >
                        ...
                        {!canAdmin && branch.id === assignedBranch?.id && (
                          <b>
        ...
        <div
          className={
            branchSwitching ? "branch-workspace is-loading" : "branch-workspace"
          }
        >
          {branchSwitching && (
            <div
              className="branch-loading-panel"
              role="status"
              aria-live="polite"
            >
          ...
          {activeView === "dashboard" && (
            <Dashboard
              ...
              alertStockTotals={notificationStockTotals}
              canAdmin={canAdmin}
          ...
          {activeView === "medicines" && (
            <Medicines
              ...
              canManagePrices={Boolean(canManagePrices)}
              canFulfillActiveBranch={Boolean(canWriteActiveBranch)}
          ...
          {activeView === "adjust" && activeBranch && (
            <Adjustments
              activeBranch={activeBranch}
              stockRows={activeBranchStockRows}
              canAdjust={canAdjust && Boolean(canWriteActiveBranch)}
              executeAction={executeAction}
              flash={flash}
            />
          )}
      ...
      <section className="login-panel auth-panel workspace-loading-panel">
        ...
        <div className="workspace-loading-company">
          ...
          <div>
            ...
            <p>
              Loading your company pharmacy file and preparing your dashboard.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="login-panel auth-panel">
        ...
        <div>
          <span className="eyebrow">
            {activeMode === "setup" ? "Totalenergies" : companyName}
          </span>
          <h1>{authTitle}</h1>
          <p>{authCopy}</p>
        </div>
    ...
    <form className="form-grid" onSubmit={submit} autoComplete="off">
      <label className="full">
        ...
        <input
          ...
          placeholder="Totalenergies Clinic Pharmacy"
          autoFocus
        />
      </label>
      <label className="full">
        Internal reference/licence details
        <input
          required
          ...
          placeholder="Enter clinic or registration reference"
        />
      </label>
      <label className="full">
        Main clinic address
        <input
          ...
          placeholder="Enter main clinic address"
        />
      </label>
      <label className="full">
        First clinic/site
        <input
          ...
          placeholder="Enter first clinic or pharmacy site"
        />
      </label>
      <label className="full">
...
function Dashboard({
  ...
  const expired = alertStockRows.filter(
    (row) => row.quantity > 0 && row.status === "expired",
  );
  const costValue = stockRows.reduce(
    (sum, row) => sum + Math.max(0, row.costValue),
    0,
  );
  const activeSkuCount = canAdmin
  ...
  const pendingUsers = canAdmin
    ? db.users.filter((user) => user.status === "pending").length
    : 0;
  const activeBranches = canAdmin
    ? getActiveBranches(db).filter((branch) =>
        canViewBranch(db, currentUser, branch.id),
      )
    : getActiveBranches(db).filter((branch) => branch.id === activeBranch?.id);
  const branchSummaries = activeBranches.map((branch) => {
    const rows = stockRows.filter(
      (row) => row.batch.branchId === branch.id && row.quantity > 0,
    );
    const branchStockValue = rows.reduce(
      (sum, row) => sum + Math.max(0, row.costValue),
      0,
    );
    const branchExpired = rows.filter((row) => row.status === "expired").length;
    const branchNearExpiry = rows.filter(
    ...
    <div className="page-grid">
      ...
      {assignedBranch && (
        <section className="content-section assignment-strip">
          ...
          {activeBranch && activeBranch.id !== assignedBranch.id && (
            <b className="pill warning">
              Currently viewing {activeBranch.code || activeBranch.name}
            </b>
          )}
        </section>
      )}
...
function Medicines({
  ...
  const stockCostTotals = useMemo(() => {
    ...
    stockRows.forEach((row) => {
      totals.set(
        row.medicine.id,
        (totals.get(row.medicine.id) ?? 0) + Math.max(0, row.costValue),
      );
    });
    return totals;
  }, [stockRows]);
  const totalCatalogCost = stockRows.reduce(
    (sum, row) => sum + Math.max(0, row.costValue),
    0,
  );
  ...
  const visible = db.medicines.filter((medicine) => {
    const text =
      `${medicine.sku} ${medicine.brandName} ${medicine.genericName} ${medicine.form} ${medicine.strength} ${medicine.category} ${medicine.nafdacNumber} ${medicine.barcodes.join(" ")}`.toLowerCase();
    return text.includes(query.toLowerCase());
  });
  const requestableBranches = getActiveBranches(db).filter(
    (branch) =>
      branch.id !== activeBranch?.id &&
      (isSuperAdmin(db, currentUser) ||
        currentUser.branchIds.includes(branch.id) ||
        currentUser.managedBranchIds.includes(branch.id)),
  );
  const requestingBranch = requestableBranches[0];
  const requestMedicine = db.medicines.find(
    (medicine) => medicine.id === requestMedicineId,
  );
  const requestBatches = stockRows
    .filter(
      (row) =>
        row.medicine.id === requestMedicineId &&
        row.quantity > 0 &&
        row.daysToExpiry >= 0,
    )
  ...
  const visibleRequisitions = db.requisitions.filter((request) => {
    const involved =
      ...
      canViewBranch(db, currentUser, request.requestingBranchId);
    const dateMatches =
      !historyDate || request.createdAt.slice(0, 10) === historyDate;
    return involved && dateMatches;
  });
  const incomingRequisitions = db.requisitions.filter(
    (request) =>
      request.status === "pending" &&
      activeBranch &&
      request.sourceBranchId === activeBranch.id &&
      (isSuperAdmin(db, currentUser) ||
        canViewBranch(db, currentUser, request.sourceBranchId)),
  );
  const awaitingReceiptRequisitions = db.requisitions.filter(
    (request) =>
      request.status === "released" &&
      activeBranch &&
      request.requestingBranchId === activeBranch.id &&
      (isSuperAdmin(db, currentUser) ||
        canViewBranch(db, currentUser, request.requestingBranchId)),
  );
  const todaysRequisitions = visibleRequisitions
    ...
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  const selectedRequisition = db.requisitions.find(
    (request) => request.id === selectedRequisitionId,
  );
  ...
  function addRequestItem() {
    ...
    const quantity = Math.max(
      1,
      Math.min(Number(requestQuantity) || 1, requestBatch.quantity),
    );
    setCartItems((current) => {
      const existing = current.find(
        (item) => item.batchId === requestBatch.batch.id,
      );
      if (existing) {
        return current.map((item) =>
          item.batchId === requestBatch.batch.id ? { ...item, quantity } : item,
        );
      }
      return [
        ...current,
  ...
  function requisitionItemAvailability(item: RequisitionItem) {
    return (
      stockRows.find((row) => row.batch.id === item.batchId)?.quantity ?? 0
    );
  }
  ...
  function updateReleaseQuantity(item: RequisitionItem, quantity: number) {
    const maxRelease = Math.min(
      item.quantity,
      requisitionItemAvailability(item),
    );
  ...
  function fulfillRequisition(request: Requisition) {
    const items = request.items.map((item) => ({
      itemId: item.id,
      quantity: getReleaseQuantity(item),
    }));
    if (!items.some((item) => item.quantity > 0)) {
      flash(
        "Release at least one item quantity before fulfilling this request",
      );
      return;
    }
    void executeAction(
      "fulfillRequisition",
      { requestId: request.id, items },
      "Requisition released for receiving confirmation",
    );
    setReleaseQuantities((current) => {
      const next = { ...current };
  ...
  function receiveRequisition(request: Requisition) {
    const items = request.items.map((item) => ({
      itemId: item.id,
      quantity: getReceiveQuantity(item),
    }));
    if (!items.some((item) => item.quantity > 0)) {
    ...
    void executeAction(
      "receiveRequisition",
      { requestId: request.id, items },
      "Requisition received and stock added to branch",
    );
    setReceiveQuantities((current) => {
      const next = { ...current };
      request.items.forEach((item) => delete next[item.id]);
  ...
  function edit(medicine: Medicine) {
    setDrafts([
      { ...medicine, rowId: id("row"), barcodes: medicine.barcodes.join(", ") },
    ]);
  }
  ...
  function updateDraft(rowId: string, updates: Partial<MedicineDraft>) {
    setDrafts((current) =>
      current.map((draft) =>
        draft.rowId === rowId ? { ...draft, ...updates } : draft,
      ),
    );
  }
  ...
  async function importFile(file: File) {
    ...
    const imported = rows
      .map((row) => ({
        ...
        form:
          getImportValue(row, ["form", "dosage form", "formulation"]) ||
          "Tablet",
        ...
        costPrice:
          Number(getImportValue(row, ["cost price", "cost", "unit cost"])) || 0,
        sellingPrice:
          Number(
            getImportValue(row, [
              "selling price",
        ...
        category: getImportValue(row, [
          "category",
          "class",
          "therapeutic class",
        ]),
        manufacturer: getImportValue(row, ["manufacturer", "maker"]),
        nafdacNumber: getImportValue(row, [
          "nafdac number",
          "nafdac no",
          "nafdac registration number",
        ]),
        barcodes: getImportValue(row, [
          "barcodes",
          "barcode",
          "ean",
          "code128",
        ...
        active:
          getImportValue(row, ["active", "status"]).toLowerCase() !==
          "inactive",
      }))
    ...
    setDrafts(imported);
    flash(
      `${imported.length} medicine row${imported.length > 1 ? "s" : ""} imported for review`,
    );
  }
  ...
  function submit(event: FormEvent) {
    ...
    if (
      records.some(
        (record) => !record.sku || !record.brandName || !record.genericName,
      )
    ) {
    ...
    if (
      records.some(
        (record) =>
          record.sellingPrice > 0 && record.sellingPrice < record.costPrice,
      )
    ) {
    ...
    const duplicateBarcode = db.medicines.find((medicine) =>
      records.some(
        (record) =>
          medicine.id !== record.id &&
          medicine.barcodes.some((code) => record.barcodes.includes(code)),
      ),
    );
    if (duplicateBarcode) {
    ...
    const duplicateIdentity = db.medicines.find((medicine) =>
      records.some(
        (record) =>
          medicine.id !== record.id &&
          medicineDuplicateKey(medicine) === medicineDuplicateKey(record),
      ),
    );
    if (duplicateIdentity) {
      flash(
        `Likely duplicate medicine: ${duplicateIdentity.brandName} already matches this brand/generic/form/strength identity`,
      );
      return;
    }
    void executeAction(
      records.length === 1 ? "upsertMedicine" : "upsertMedicines",
      records.length === 1
        ? { record: records[0], branchId: activeBranch?.id }
        : { records, branchId: activeBranch?.id },
      `${records.length} medicine record${records.length > 1 ? "s" : ""} saved`,
    );
    resetDrafts();
  }

  function updateMedicinePricing(
    medicine: Medicine,
    updates: Partial<Pick<Medicine, "costPrice" | "sellingPrice">>,
  ) {
    ...
    const overrideReason = autoPricingOn
      ? window
          .prompt(
            "Auto pricing is enabled. Enter the admin override reason for this manual price change.",
          )
          ?.trim()
      : "";
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Capture NAFDAC number, barcode, dosage form, reorder level, and
              manufacturer.
            </p>
          </div>
          <div className="value-summary">
            <Archive size={17} />
            <strong>
              {activeBranch ? `${activeBranch.code} value` : "Branch value"}
            </strong>
          ...
          <button
            className="ghost-button cart-button"
            type="button"
              ...
              {visible.length ? (
                visible.map((medicine) => (
                  <tr key={medicine.id}>
                    ...
                    <td>
                      <strong>
                        {number.format(stockTotals.get(medicine.id) ?? 0)}
                      </strong>
                      <span className="table-subtext">
                        {sellableUnitLabel(medicine)}
                      </span>
                    </td>
                    <td>
                      <input
                        ...
                        onBlur={(event) =>
                          updateMedicinePricing(medicine, {
                            costPrice: Number(event.currentTarget.value) || 0,
                          })
                        }
                        disabled={
                          !canManagePrices ||
                          !activeBranch ||
                          (autoPricingOn && !canOverrideAutoPricing)
                        }
                    ...
                    <td>
                      <input
                        ...
                        defaultValue={
                          ...
                          getMedicineSellingPrice(stockRows, medicine.id) ||
                          ""
                        }
                        onBlur={(event) =>
                          updateMedicinePricing(medicine, {
                    ...
                    <td>
                      {money.format(stockCostTotals.get(medicine.id) ?? 0)}
                    </td>
                    <td>
                      <span
                        className={medicine.active ? "pill good" : "pill muted"}
                      >
                    ...
                    <td>
                      <div className="table-actions">
                        ...
                        <button
                          className="icon-button"
                          type="button"
                          onClick={() => openRequestModal(medicine.id)}
                          title="Request from this branch"
                          disabled={
                            !activeBranch ||
                            !requestableBranches.length ||
                            (stockTotals.get(medicine.id) ?? 0) <= 0
                          }
                        >
                ...
                <tr>
                  <td colSpan={11}>
                    No medicine records yet. Add the pharmacy catalog before
                    receiving stock.
                  </td>
                </tr>
              )}
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Add medicines one by one, use the plus button for multiple rows,
        ...
        <form className="form-grid" onSubmit={submit}>
                ...
                <div className="form-grid">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      value={draft.sku}
                      onChange={(event) =>
                        updateDraft(draft.rowId, { sku: event.target.value })
                      }
                      disabled={!canWrite}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateDraft(draft.rowId, {
                          nafdacNumber: event.target.value,
                        })
                      }
                      disabled={!canWrite}
                    />
                  ...
                  <label>
                    ...
                    <input
                      value={draft.form}
                      onChange={(event) =>
                        updateDraft(draft.rowId, { form: event.target.value })
                      }
                      disabled={!canWrite}
                    />
                  </label>
                  ...
                  <label>
                    ...
                    <input
                      value={draft.unit}
                      onChange={(event) =>
                        updateDraft(draft.rowId, { unit: event.target.value })
                      }
                      placeholder="Pack, bottle, carton"
                      disabled={!canWrite}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="number"
                      min="1"
                      value={numberInputValue(draft.packSize)}
                      onChange={(event) =>
                        updateDraft(draft.rowId, {
                  ...
                  <label>
                    Least sellable unit
                    <input
                      value={draft.sellableUnit}
                      onChange={(event) =>
                        updateDraft(draft.rowId, {
                          sellableUnit: event.target.value,
                        })
                      }
                      placeholder="Tablet, sachet, capsule"
                      disabled={!canWrite}
                    />
                  </label>
                  <label>
                    Cost price / least unit
                    <input
                      type="number"
                      min="0"
                      ...
                      disabled={!canWrite || !canManagePrices}
                    />
                  </label>
                  <label>
                    Selling price / least unit
                    <input
                      type="number"
                      min="0"
                      value={numberInputValue(draft.sellingPrice)}
                      onChange={(event) =>
                        updateDraft(draft.rowId, {
                          sellingPrice: Number(event.target.value),
                        })
                      }
                      disabled={!canWrite || !canManagePrices || autoPricingOn}
                    />
                  </label>
                  <label>
                    Category
                  ...
                  <label>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={numberInputValue(draft.reorderLevel)}
                      onChange={(event) =>
                        updateDraft(draft.rowId, {
          ...
          <div className="form-actions full">
            ...
            >
              ...
              Save{" "}
              {drafts.length > 1 ? `${drafts.length} medicines` : "medicine"}
            </button>
          </div>
        </form>
      </section>
          ...
          <section className="modal-panel">
            <div className="section-heading">
              <div>
                ...
                <p>
                  Requesting from {activeBranch?.name} into{" "}
                  {requestingBranch?.name ?? "your branch"}.
                </p>
              </div>
              <button
                className="icon-button"
            ...
            <div className="stack">
              <div className="availability">
                ...
                <span>
                  ...
                  {number.format(requestBatch.quantity)} available
                </span>
              </div>
              <label>
                Quantity requested
                <input
                  type="number"
                  min="1"
                  max={requestBatch.quantity}
                  value={numberInputValue(requestQuantity)}
                  onChange={(event) =>
          ...
          <section className="modal-panel wide">
            <div className="section-heading">
              <div>
                ...
                <p>
                  {activeBranch
                    ? `Requests from ${activeBranch.name}`
                    : "Branch request cart"}{" "}
                  are sent to the supplying branch team.
                </p>
              </div>
              <button
                className="icon-button"
            ...
            <section className="stack requisition-today-panel">
              ...
              {todaysRequisitions.length ? (
                todaysRequisitions.map((request) => (
                  <button
                    className="line-card requisition-summary-card"
                    type="button"
                    key={request.id}
                    onClick={() => setSelectedRequisitionId(request.id)}
                  >
                    <strong>
                      {getBranchName(db, request.requestingBranchId)} from{" "}
                      {getBranchName(db, request.sourceBranchId)}
                    </strong>
                    <span>
                      {requisitionStatusLabel(request.status)} /{" "}
                    ...
                    <span>
                      Requester: {getUserName(db, request.requesterUserId)} /
                      Fulfiller:{" "}
                      {getUserName(db, request.releasedBy ?? request.handledBy)}{" "}
                      / Receiver: {getUserName(db, request.receivedBy)}
                    </span>
                  </button>
                ))
              ) : (
                <div className="empty-state">
                  No requisitions recorded today.
                </div>
              )}
            </section>
            ...
            <div className="cart-layout">
              <section className="stack">
                  ...
                  cartRows.map((item) => (
                    <article className="line-card" key={item.rowId}>
                      ...
                      <span>
                        ...
                        {item.row
                          ? `${number.format(item.row.quantity)} available`
                      ...
                      <div className="button-row">
                        <label>
                          ...
                          <input
                            type="number"
                            min="1"
                            max={item.row?.quantity ?? undefined}
                ...
                <div className="form-actions">
                  <button
                    ...
                    type="button"
                    onClick={submitCart}
                    disabled={!cartItems.length || !requestingBranch}
                  >
                    <Send size={17} />
                    Send request
                  </button>
              ...
              <section className="stack">
                ...
                {incomingRequisitions.length ? (
                  incomingRequisitions.map((request) => (
                    <article className="line-card" key={request.id}>
                      <button
                        className="requisition-open-row"
                        type="button"
                        onClick={() => setSelectedRequisitionId(request.id)}
                      >
                        <strong>
                          {getBranchName(db, request.requestingBranchId)}{" "}
                          request
                        </strong>
                        <span>
                      ...
                      {request.items.map((item) => {
                        ...
                        return (
                          <div
                            className="requisition-release-row"
                            key={item.id}
                          >
                            <span>
                              {medicine?.brandName ?? item.medicineId} /{" "}
                              {batch?.batchNumber ?? item.batchId}
                              <small>
                                Requested {number.format(item.quantity)} /
                                available {number.format(available)}{" "}
                                {medicine
                            ...
                            <label>
                              ...
                              <input
                                ...
                                value={numberInputValue(
                                  getReleaseQuantity(item),
                                )}
                                onChange={(event) =>
                                  updateReleaseQuantity(
                                    item,
                                    Number(event.target.value),
                                  )
                                }
                      ...
                      <div className="button-row">
                        <button
                          className="primary-button"
                          type="button"
                          onClick={() => fulfillRequisition(request)}
                          disabled={!canFulfillActiveBranch}
                        >
                        ...
                        <button
                          className="ghost-button"
                          type="button"
                          onClick={() => rejectRequisition(request.id)}
                          disabled={!canFulfillActiveBranch}
                        >
                          <XCircle size={16} />
                          Reject
                  ...
                  <div className="empty-state">
                    No incoming requests for this branch.
                  </div>
                )}
              </section>
              ...
              <section className="stack">
                ...
                {awaitingReceiptRequisitions.length ? (
                  awaitingReceiptRequisitions.map((request) => (
                    <button
                      className="line-card requisition-summary-card"
                      type="button"
                      key={request.id}
                    ...
                    >
                      <strong>
                        {getBranchName(db, request.sourceBranchId)} released
                        stock
                      </strong>
                      <span>
                        {request.items.length} item
                        {request.items.length === 1 ? "" : "s"} / released by{" "}
                        {getUserName(
                          db,
                          request.releasedBy ?? request.handledBy,
                        )}
                      </span>
                      <span>
                        Requester: {getUserName(db, request.requesterUserId)} /
                  ...
                  <div className="empty-state">
                    No released requisitions awaiting receipt.
                  </div>
                )}
              </section>
            </div>
              ...
              {historyOpen && (
                <div className="stack">
                  ...
                  {visibleRequisitions.length ? (
                        ...
                        {request.items.map((item) => {
                          ...
                          return (
                            <span key={item.id}>
                              {medicine?.brandName ?? item.medicineId} /
                              requested {number.format(item.quantity)}
                              {releasedQuantity(item)
                                ? ` / released ${number.format(releasedQuantity(item))}`
                                : ""}
                              {item.receivedQuantity !== undefined
                                ? ` / received ${number.format(item.receivedQuantity)}`
                                : ""}
                            </span>
                          );
                        })}
                    ...
                    <div className="empty-state">
                      No requisition history for this filter.
                    </div>
                  )}
                </div>
              )}
          ...
          <section className="modal-panel wide">
            <div className="section-heading">
              <div>
                ...
                <p>
                  {getBranchName(db, selectedRequisition.requestingBranchId)}{" "}
                  from {getBranchName(db, selectedRequisition.sourceBranchId)} /{" "}
                  {requisitionStatusLabel(selectedRequisition.status)}
                </p>
              </div>
              <button
                className="icon-button"
                type="button"
                onClick={() => setSelectedRequisitionId("")}
                title="Close"
              >
                <X size={17} />
              </button>
            ...
            <div className="requisition-detail-grid">
              <div>
                <span>Requester</span>
                <strong>
                  {getUserName(db, selectedRequisition.requesterUserId)}
                </strong>
                <small>
                ...
                <strong>
                  {getUserName(
                    ...
                    selectedRequisition.releasedBy ??
                      selectedRequisition.handledBy,
                  )}
                </strong>
                <small>
                  {selectedRequisition.releasedAt ||
                  selectedRequisition.handledAt
                    ? new Date(
                        selectedRequisition.releasedAt ??
                          selectedRequisition.handledAt ??
                          "",
              ...
              <div>
                <span>Receiver</span>
                <strong>
                  {getUserName(db, selectedRequisition.receivedBy)}
                </strong>
                <small>
                  {selectedRequisition.receivedAt
                    ? new Date(selectedRequisition.receivedAt).toLocaleString()
                    : "Not received yet"}
                </small>
              </div>
              <div>
                ...
                <small>
                  Supplying:{" "}
                  {getBranchName(db, selectedRequisition.sourceBranchId)}
                </small>
              </div>
            </div>
            ...
            <div className="stack">
              ...
              {selectedRequisition.items.map((item) => {
                ...
                const canReceiveThis =
                  selectedRequisition.status === "released" &&
                  activeBranch?.id === selectedRequisition.requestingBranchId &&
                  canFulfillActiveBranch;
                return (
                  <article className="requisition-detail-item" key={item.id}>
                    <div>
                      ...
                      <span>
                        ...
                        {batch?.expiryDate ?? "not recorded"} /{" "}
                        {batch?.location ?? "source branch"}
                      </span>
                    </div>
                    <dl>
                      <div>
                        <dt>Requested</dt>
                        <dd>{number.format(item.quantity)}</dd>
                      </div>
                      <div>
                        <dt>Released</dt>
                        <dd>{number.format(releasedQuantity(item))}</dd>
                      </div>
                      <div>
                        <dt>Received</dt>
                        <dd>
                    ...
                    {canReceiveThis && (
                      <label>
                        ...
                        <input
                          type="number"
                          min="0"
                          max={releasedQuantity(item)}
                          value={numberInputValue(getReceiveQuantity(item))}
                          onChange={(event) =>
            ...
            {selectedRequisition.note && (
              <div className="audit-note">Note: {selectedRequisition.note}</div>
            )}
              ...
              activeBranch?.id === selectedRequisition.requestingBranchId && (
                <div className="form-actions">
                  ...
                  <button
                    className="primary-button"
                    type="button"
                    onClick={() => receiveRequisition(selectedRequisition)}
                    disabled={!canFulfillActiveBranch}
                  >
                    <PackageCheck size={17} />
...
function PendingMedicationRecords({
  ...
  const stockTotals = useMemo(
    () =>
      aggregateMedicineStock(
        stockRows.filter(
          (row) => !activeBranch || row.batch.branchId === activeBranch.id,
        ),
      ),
    [activeBranch, stockRows],
  );
  ...
  const openCount = branchRecords.filter(
    (item) =>
      item.status === "pending" ||
      item.status === "available" ||
  ...
  function updateStatus(
    ...
    if (
      status === "fulfilled" &&
      !window.confirm(
        "Mark this request fulfilled? Actual dispensing should still be recorded in Prescriptions so stock deduction remains accurate.",
      )
    )
      return;
    if (
      status === "cancelled" &&
      !window.confirm("Cancel this pending medication request?")
    )
      return;
    void executeAction(
      "updatePendingMedication",
      { pendingMedicationId: item.id, status },
    ...
    <section className="content-section pending-medication-section full">
      <div className="section-heading">
        ...
        <div className="pending-medication-summary">
          <span>
            <strong>{number.format(openCount)}</strong> open
          </span>
          <span>
      ...
      <div className="pending-medication-list">
        {branchRecords.length ? (
          branchRecords.map((item) => {
            ...
            return (
              <article
                className={`pending-medication-card is-${item.status}`}
                key={item.id}
              >
                <header>
                  <div>
                    ...
                    <span>
                      {[item.genericName, item.strength, item.form]
                        .filter(Boolean)
                        .join(" / ") || "Unavailable medication request"}
                    </span>
                  </div>
                ...
                <dl>
                  ...
                  <div>
                    <dt>Requested</dt>
                    <dd>
                      {number.format(item.quantity)} {item.unit || "unit"} /{" "}
                  ...
                  <div>
                    <dt>Recorded by</dt>
                    <dd>{getUserName(db, item.recordedBy)}</dd>
                  </div>
                  <div>
                    <dt>Source</dt>
                    <dd>
                      {item.sourceNote || "Prescription unavailable item"}
                    </dd>
                  </div>
                </dl>
                {item.status === "available" && (
                  <div className="form-success">
                    Available since{" "}
                    {new Date(
                      item.availableAt ?? item.updatedAt,
                    ).toLocaleString()}
          ...
          <div className="empty-state">
            No pending medication records yet. Unavailable prescription items
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Supplier license references support receiving traceability and
              compliance review.
            </p>
          </div>
        </div>
        <div className="supplier-grid">
          {db.suppliers.length ? (
            ...
            <div className="empty-state">
              No suppliers yet. Add approved suppliers before receiving stock.
            </div>
          )}
        </div>
      </section>
      ...
      <section className="content-section">
        ...
        <form className="form-grid" onSubmit={submit}>
          ...
          <label>
            License reference
            <input
              value={form.licenseRef}
              onChange={(event) =>
                setForm({ ...form, licenseRef: event.target.value })
              }
              disabled={!canWrite}
            />
          </label>
...
function ReceiveStock({
  ...
  const hasReceivableItems =
    db.medicines.some((medicine) => medicine.active) ||
    db.products.some((product) => product.active);
  const policy = pricingPolicy(db.settings);

  function lineUnitCost(
    line: Pick<ReceiveLine, "itemType" | "itemId" | "unitCost">,
  ) {
    if (line.itemType === "product")
      return Math.max(0, Number(line.unitCost) || 0);
  ...
  function updateLine(rowId: string, updates: Partial<ReceiveLine>) {
    setLines((current) =>
      current.map((line) =>
        line.rowId === rowId ? applyAutoPrice({ ...line, ...updates }) : line,
      ),
    );
  }

  function itemPriceDefaults(
    itemType: ReceiveLine["itemType"],
    itemId: string,
  ) {
    if (itemType === "product") {
    ...
    return {
      unitCost: medicine
        ? (medicine.costPrice || 0) * medicineUnitsPerContainer(medicine)
        : 0,
      ...
      current.map((line) =>
        line.rowId === rowId
          ? applyAutoPrice({
              ...
              itemType,
              itemId,
              unitCost: line.unitCost || defaults.unitCost,
              sellingPrice: line.sellingPrice || defaults.sellingPrice,
            })
          : line,
      ),
    );
  ...
  function buildReviewLines() {
    if (!selectedSupplierId || !hasReceivableItems || !activeBranch.id) {
      flash(
        "Add at least one medicine, supplier, and clinic site before receiving stock",
      );
      return [];
    }
    const items = lines.map((line) => {
      const fallbackId =
        line.itemType === "product"
          ? db.products.find((product) => product.active)?.id
          : db.medicines.find((medicine) => medicine.active)?.id;
      ...
      const unitCost =
        line.itemType === "medicine"
          ? medicineUnitCostFromContainerCost(
              item as Medicine | undefined,
              Number(line.unitCost),
            )
          : Math.max(0, Number(line.unitCost) || 0);
      const priceCalc = calculatedSellingPrice(
        db,
        line.itemType,
        itemId,
        unitCost,
      );
      const sellingPrice = policy.enabled
        ? priceCalc.price
        : Math.max(0, Number(line.sellingPrice) || 0);
      const containers = Math.max(0, Number(line.quantity) || 0);
      const unitsPerContainer =
        line.itemType === "medicine"
          ? medicineUnitsPerContainer(item as Medicine | undefined)
          : 1;
      const postedQuantity = containers * unitsPerContainer;
      const existingCost = item ? Math.max(0, Number(item.costPrice) || 0) : 0;
      const costDelta =
        existingCost > 0 && unitCost > 0
          ? (Math.abs(unitCost - existingCost) / existingCost) * 100
          : 0;
      const warnings = [
        sellingPrice > 0 && sellingPrice < unitCost
          ? "Selling price is below cost"
          : "",
        priceCalc.markupPercent > policy.unusualMarkupPercent
          ? `Markup is above ${policy.unusualMarkupPercent}%`
          : "",
        costDelta > policy.costChangeWarningPercent
          ? `Cost changed ${Math.round(costDelta)}% from catalog cost`
          : "",
        line.itemType === "medicine" &&
        unitsPerContainer <= 1 &&
        (item as Medicine | undefined)?.unit?.toLowerCase() !==
          (item as Medicine | undefined)?.sellableUnit?.toLowerCase()
          ? "Units per container is 1; confirm package setup"
          : "",
      ].filter(Boolean);
      return {
        ...line,
        ...
        itemName: item
          ? line.itemType === "product"
            ...
            : medicineOptionLabel(item as Medicine)
          : "Unknown item",
        unitLabel:
          line.itemType === "medicine"
            ? medicineSellableUnit(item as Medicine | undefined)
            : (item as Product | undefined)?.unit || "unit",
        containers,
        unitsPerContainer,
        postedQuantity,
    ...
    if (
      items.some(
        (line) =>
          line.sellingPrice > 0 && line.sellingPrice < line.leastUnitCost,
      )
    ) {
    ...
    <section className="content-section">
      <div className="section-heading">
        <div>
          ...
          <p>
            Posting into {activeBranch.name}. Switch branch from the top bar
            before receiving elsewhere.
          </p>
        </div>
        <button
          className="ghost-button"
      ...
      {!canWrite && (
        <div className="form-error">
          You only have view access in {activeBranch.name}. Ask the branch
          manager or an admin for posting access.
        </div>
      )}
      <form className="form-grid" onSubmit={submit}>
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              setHeader({ ...header, supplierId: event.target.value })
            }
            disabled={!canWrite || !db.suppliers.length}
          >
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              setHeader({ ...header, invoiceRef: event.target.value })
            }
            disabled={!canWrite}
          />
        </label>
        <label className="scan-field full">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Search by item name, SKU, or barcode, then press Enter"
              disabled={!canWrite || !hasReceivableItems}
            />
            <button
              className="ghost-button"
              type="button"
        ...
        <div className="line-editor full">
          {lines.map((line, index) => {
            const medicineOptions = db.medicines.filter(
            ...
            const selectedMedicine = selectedMedicineForLine({
              ...line,
              itemId: selectedItemId,
            });
            const lineSummary = receiveLineSummary({
              ...line,
              itemId: selectedItemId,
            });
            return (
                ...
                <div className="form-grid">
                  <label className="full">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        selectItem(line.rowId, "medicine", event.target.value)
                      }
                      disabled={!canWrite || !hasItemOptions}
                    >
                  ...
                  <label>
                    {`Containers received${selectedMedicine ? ` (${medicineContainerPackage(selectedMedicine)})` : ""}`}
                    <input
                      required
                      type="number"
                      min="1"
                  ...
                  <label>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={numberInputValue(line.unitCost)}
                      onChange={(event) =>
                        updateLine(line.rowId, {
                  ...
                  <label>
                    ...
                    <input
                      type="number"
                      min="0"
                      value={numberInputValue(line.sellingPrice)}
                      onChange={(event) =>
                        updateLine(line.rowId, {
                          sellingPrice: Number(event.target.value),
                        })
                      }
                      disabled={!canWrite || policy.enabled}
                    />
                  </label>
                  <label>
                    Stock location
                    <input
                      value={line.location}
                      onChange={(event) =>
                        updateLine(line.rowId, { location: event.target.value })
                      }
                      disabled={!canWrite}
                    />
                  </label>
                  {lineSummary && (
                    <div className="receive-conversion-note full">
                      <div>
                        <strong>
                          {number.format(line.quantity || 0)}{" "}
                          {lineSummary.containerPackage} x{" "}
                          {number.format(lineSummary.unitsPerContainer)}{" "}
                          {lineSummary.leastUnit}
                        </strong>
                        <span>
                          = {number.format(lineSummary.totalLeastUnits)}{" "}
                          {lineSummary.leastUnit} posted to stock
                        </span>
                      </div>
                      <div>
        ...
        <div className="form-actions full">
          ...
          <button
            className="primary-button"
            type="submit"
            disabled={!canWrite || !hasReceivableItems || !db.suppliers.length}
          >
            <PackagePlus size={17} />
      ...
      {reviewOpen && (
        ...
        >
          <section
            className="pos-modal-panel receive-review-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="pos-modal-header">
              ...
              <button
                ...
                aria-label="Close receive review"
              >
                <X size={18} />
              </button>
            </header>
            <div className="receive-review-summary">
              <div>
                ...
                <strong>{number.format(reviewLines.length)}</strong>
              </div>
              <div>
                ...
                <strong>
                  {number.format(
                    reviewLines.reduce(
                      (sum, line) => sum + line.postedQuantity,
                      0,
                    ),
                  )}
                </strong>
              ...
              <div>
                ...
                <strong>
                  {money.format(
                    reviewLines.reduce((sum, line) => sum + line.lineCost, 0),
                  )}
                </strong>
              </div>
              <div>
                ...
                <strong>
                  {money.format(
                    reviewLines.reduce((sum, line) => sum + line.lineRetail, 0),
                  )}
                </strong>
              </div>
            </div>
            {policy.enabled && (
              <div className="receive-review-policy">
                <Sparkles size={16} />
                Auto pricing is on. Selling prices below are calculated from
                least-unit cost, markup priority, and rounding.
              </div>
            )}
            {reviewLines.some((line) => line.warnings.length) && (
              <div className="receive-review-warnings">
                ...
                <div>
                  <strong>
                    {reviewLines.reduce(
                      (sum, line) => sum + line.warnings.length,
                      0,
                    )}{" "}
                    warning
                    {reviewLines.reduce(
                  ...
                  <span>
                    Confirm the highlighted line values before posting stock.
                  </span>
                </div>
              </div>
            )}
              ...
              <table>
                ...
                <tbody>
                  {reviewLines.map((line) => (
                    <tr
                      key={line.rowId}
                      className={line.warnings.length ? "warn-row" : ""}
                    >
                      <td>
                        <strong>{line.itemName}</strong>
                        <span className="table-subtext">
                          {line.batchNumber || "No batch"} /{" "}
                          {line.expiryDate || "No expiry"} / {line.location}
                        </span>
                      </td>
                      <td>
                        {number.format(line.containers)}{" "}
                        {line.itemType === "medicine"
                          ? medicineContainerPackage(
                              db.medicines.find(
                                (medicine) => medicine.id === line.itemId,
                              ),
                            )
                          : line.unitLabel}
                      </td>
                      <td>
                        {number.format(line.postedQuantity)} {line.unitLabel}
                      </td>
                      ...
                      <td>
                        <strong>{money.format(line.sellingPrice)}</strong>
                      </td>
                      <td>
                        {line.warnings.length ? (
                          ...
                          <span className="success-chip">Checked</span>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="form-actions">
              ...
              <button
                className="primary-button"
                type="button"
                onClick={() => postReviewedLines(reviewLines)}
                disabled={!reviewLines.length}
              >
                <PackageCheck size={17} />
...
function IssueStock({
  ...
  const availableBatches = stockRows
    .filter(
      (row) =>
        row.batch.branchId === selectedBranchId &&
        row.medicine.id === selectedMedicineId &&
        row.quantity > 0 &&
        row.daysToExpiry >= 0,
    )
    .sort((a, b) => a.batch.expiryDate.localeCompare(b.batch.expiryDate));
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            <h2>Stock Issue</h2>
            <p>
              Issuing from {activeBranch.name}. Expired batches are excluded and
              FEFO is applied.
            </p>
          </div>
        </div>
        {!canWrite && (
          <div className="form-error">
            You only have view access in {activeBranch.name}. Ask the branch
            manager or an admin for stock-out access.
          </div>
        )}
        <form className="form-grid" onSubmit={submit}>
          <label className="scan-field full">
            ...
            <div>
              ...
              <input
                ...
                disabled={!canWrite || !db.medicines.length}
              />
              <button
                className="ghost-button"
                type="button"
          ...
          <label className="full">
            ...
            <input
              ...
              placeholder="Receipt, request, or memo reference"
              disabled={!canWrite}
            />
          </label>
          <div className="availability full">
            ...
            <span>
              available from non-expired batches in{" "}
              {getBranchName(db, selectedBranchId)}
            </span>
          </div>
          <div className="form-actions full">
            <button
              ...
              disabled={
                !canWrite ||
                !selectedMedicineId ||
                Number(form.quantity) > totalAvailable ||
                totalAvailable <= 0
              }
            >
...
function POSView({
  ...
  const frequentlySoldOptions = useMemo(() => {
    ...
    branchSales.forEach((sale) => {
      sale.items.forEach((item) => {
        const itemId =
          item.itemType === "product" ? item.productId : item.medicineId;
        if (!itemId) return;
        ...
        totals.set(key, {
          ...
          lastSoldAt:
            current && current.lastSoldAt > sale.soldAt
              ? current.lastSoldAt
              : sale.soldAt,
        });
      });
    ...
    return allSaleOptions
      .map((option) => ({
        option,
        sales: totals.get(`${option.itemType}:${option.itemId}`),
      }))
      .filter((entry) => entry.sales && entry.option.available > 0)
      .sort(
        (a, b) =>
          (b.sales?.quantity ?? 0) - (a.sales?.quantity ?? 0) ||
          (b.sales?.lastSoldAt ?? "").localeCompare(a.sales?.lastSoldAt ?? ""),
      )
  ...
  const saleOptions = allSaleOptions
    .filter((item) => {
      const text =
        `${item.title} ${item.meta} ${item.category} ${item.scanCodes.join(" ")}`.toLowerCase();
      return text.includes(query.toLowerCase());
    })
    .slice(0, 16);
  ...
  const pendingCartRows = cartRows.filter(
    (item) => item.itemType === "medicine" && item.available <= 0,
  );
  const filteredSales = branchSales.filter((sale) => {
    const soldDate = sale.soldAt.slice(0, 10);
    return (
      (!historyStartDate || soldDate >= historyStartDate) &&
      (!historyEndDate || soldDate <= historyEndDate)
    );
  });
  const filteredSalesTotal = filteredSales.reduce(
    (sum, sale) => sum + (sale.total ?? sale.subtotal),
    0,
  );
  const filteredSalesItems = filteredSales.reduce(
    (sum, sale) =>
      sum + sale.items.reduce((itemSum, item) => itemSum + item.quantity, 0),
    0,
  );
  const filteredPatientCount = new Set(
    filteredSales
      .map(
        (sale) =>
          normalizePhone(sale.customerPhone) ||
    ...
    return patientProfiles
      .filter(
        (profile) =>
          ...
          normalizePhone(profile.phone) !== activePhone,
      )
      .slice(0, 5);
  ...
  const phoneMatches = useMemo(() => {
    ...
    if (selectedPatient && normalizePhone(selectedPatient.phone) === phone)
      return [];
    return patientProfiles
  ...
  function addItem(itemType: CartItem["itemType"], itemId: string) {
        ...
        return current.map((item) =>
          item.rowId === existing.rowId
            ? option.available > 0
              ? {
                  ...item,
                  quantity: Math.min(item.quantity + 1, option.available),
              ...
              : {
                  ...item,
                  quantity: 0,
                  requestedQuantity: (item.requestedQuantity ?? 1) + 1,
                }
            : item,
        );
      }
    ...
    flash(
      option.available > 0
        ? `${option.title} added to prescription basket`
        : `${option.title} added as pending medication`,
    );
  }

  function applyScan() {
    const needle = scan.trim().toLowerCase();
    const option = allSaleOptions.find((item) =>
      item.scanCodes.some((code) => code.toLowerCase() === needle),
    );
    if (!option) {
      flash("No medicine found for scanned code");
      return;
  ...
  function optionTone(option: SaleOption) {
    if (option.itemType === "medicine") {
      ...
      if (earliest && earliest.daysToExpiry <= db.settings.nearExpiryDays)
        return "fefo";
      if (
        medicine &&
        option.available > 0 &&
        option.available <= medicine.reorderLevel
      )
        return "low";
    }
    return "";
  ...
  const tillTime = new Date().toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
    ...
    setCart((current) =>
      current.map((item) => {
        ...
        const nextQuantity = Math.max(
          0,
          Math.min(
            Number(updates.quantity ?? item.quantity) || 0,
            available || 0,
          ),
        );
        return {
          ...
          requestedQuantity:
            available > 0
              ? Math.max(requestedQuantity, nextQuantity)
              : requestedQuantity,
        };
      }),
    );
  ...
  function toggleCounselingSuggestion(
    rowId: string,
    suggestion: CounselingSuggestion,
  ) {
    setCart((current) =>
      current.map((item) =>
        item.rowId === rowId
          ? {
              ...item,
              labelInstruction: toggleInstruction(
                item.labelInstruction,
  ...
  function selectPatientProfile(
    profile: ReturnType<typeof buildPatientProfiles>[number],
  ) {
  ...
  function buildMedicationLabels() {
    return cartRows
      .filter(
        (item) =>
          item.itemType === "medicine" && item.option && item.quantity > 0,
      )
      .map((item) => {
        const medicine = db.medicines.find((entry) => entry.id === item.itemId);
        const instruction =
          item.labelInstruction?.trim() ||
          (medicine
            ? defaultLabelInstruction(db, "medicine", medicine.id)
            : "");
        return {
          medicineName: item.option?.title ?? medicine?.brandName ?? "Medicine",
          medicineMeta: medicine
  ...
  function printMedicationLabels(
    labels: ReturnType<typeof buildMedicationLabels>,
  ) {
    const win = window.open("", "_blank", "width=900,height=700");
    if (!win) {
      flash(
        "Medication labels are ready, but the browser blocked the print window",
      );
      return;
    }
    const patient =
      customerName.trim() || customerPhone.trim() || "Walk-in patient";
  ...
  async function submit(event: FormEvent) {
    ...
    if (
      pendingCartRows.length &&
      (!customerName.trim() || !customerPhone.trim())
    ) {
      flash(
        "Patient name and phone are required for Pending Medication follow-up",
      );
      return;
    }
    ...
    if (cartRows.some((item) => item.quantity > item.available)) {
      flash(
        "Dispensing blocked: one or more quantities exceed available stock",
      );
      return;
    }
    if (
      cartRows.some(
        (item) => item.quantity > 0 && (!item.option || item.unitPrice <= 0),
      )
    ) {
    ...
    const successMessage = cartRows.some((item) => item.quantity > 0)
      ? pendingCartRows.length
        ? "Dispensed available items and saved pending medication"
        : "Dispensed and cost captured"
  ...
  return (
    <div className="pos-redesign">
      <div className="pos-window">
        <div className="pos-appbar">
          <div className="pos-appbar-left">
            <span className="pos-title-icon">
              <Pill size={15} />
            </span>
            <strong>Prescriptions</strong>
            <span className="pos-branch-meta">
              / {db.settings.accountName} / {activeBranch.name}
            </span>
          </div>
          <div className="pos-appbar-right">
            <span className="pos-shift-pill">
              <span /> Clinic shift open / {currentUser.name}
            </span>
            <span className="pos-time-pill">
              <Clock3 size={13} /> {tillTime}
            </span>
            <button
              ...
              aria-label="View dispensing history"
            >
              <History size={16} />
            </button>
          </div>
        ...
        {!canSell && (
          <div className="form-error pos-access-error">
            You need pharmacist, site manager, or admin access in{" "}
            {activeBranch.name} to use Prescriptions.
          </div>
        )}
          ...
          <section className="pos-catalog-panel">
            ...
            <div className="pos-quick-heading">
              <div>
                ...
                <span>
                  {query.trim()
                    ? "Matching medicines, including unavailable items"
                    : "Based on real dispensing history and current stock availability"}
                </span>
              </div>
            </div>
              ...
              {visibleSaleOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => addItem(option.itemType, option.itemId)}
                    disabled={
                      !canSell ||
                      (option.available > 0 && option.unitPrice <= 0)
                    }
                  >
                    <div>
                      <strong>{option.title}</strong>
                      {option.available <= 0 && (
                        <span className="pos-card-tag danger">Pending</span>
                      )}
                      {tone === "fefo" && (
                        <span className="pos-card-tag danger">
                          <AlertTriangle size={11} /> FEFO
                        </span>
                      )}
                      {tone === "low" && (
                        <span className="pos-card-tag warn">Low</span>
                      )}
                    </div>
              ...
              {!visibleSaleOptions.length && (
                <div className="empty-state">
                  {query.trim()
                    ...
                    : "No frequently dispensed medicines yet. Use search or scan to record the first medicines."}
                </div>
              )}
            </div>
          </section>
          ...
          <aside className="pos-sale-cart">
            ...
            <div className="pos-cart-lines">
              {cartRows.map((item) => {
                const medicine =
                  item.itemType === "medicine"
                    ? db.medicines.find((entry) => entry.id === item.itemId)
                    : undefined;
                ...
                return (
                  <div className="pos-line-item" key={item.rowId}>
                    <div className="pos-line-avatar">
                      {optionInitials(item.option?.title ?? "Rx")}
                    </div>
                    <div className="pos-line-main">
                      <div className="pos-line-top">
                        <div>
                          <strong>
                            {item.option?.title ?? "Unknown item"}
                          </strong>
                          {item.itemType === "medicine" &&
                            item.available > 0 && (
                              <span className="pos-fefo-chip">FEFO</span>
                            )}
                          {item.itemType === "medicine" &&
                            item.available <= 0 && (
                              <span className="pos-card-tag danger">
                          ...
                          <small>
                            {item.option?.meta ?? "Unavailable item"} /{" "}
                            {number.format(item.available)} avail.
                          </small>
                        </div>
                        <button
                          ...
                          onClick={() =>
                            setCart((current) =>
                              current.filter(
                                (cartItem) => cartItem.rowId !== item.rowId,
                              ),
                            )
                          }
                          title="Remove item"
                      ...
                      <div className="pos-line-bottom">
                        {item.available > 0 ? (
                          <div className="pos-qty-stepper">
                            <button
                              ...
                              disabled={!canSell || item.quantity <= 1}
                            >
                              <Minus size={14} />
                            </button>
                            <input
                            ...
                            <button
                              ...
                              disabled={
                                !canSell || item.quantity >= item.available
                              }
                            >
                              <Plus size={14} />
                            </button>
                          ...
                          <label className="pos-pending-quantity">
                            ...
                            <input
                              aria-label="Pending medication quantity needed"
                              type="number"
                              min="1"
                              value={numberInputValue(
                        ...
                        <div>
                          <small>
                            {item.available > 0
                              ? `${money.format(item.unitPrice)} / ${number.format(item.available)} avail.`
                              : "0 supplied / saved to Pending Medication"}
                          </small>
                          <strong>
                            {item.available > 0
                        ...
                        <div className="pos-care-fields">
                          <label>
                            ...
                            <input
                              type="number"
                              min="0"
                              value={numberInputValue(item.daysSupply)}
                              onChange={(event) =>
                                updateCart(item.rowId, {
                                  daysSupply: Number(event.target.value),
                                })
                              }
                              disabled={!canSell}
                            />
                          ...
                          <label>
                            ...
                            {suggestions.length > 0 && (
                              <div
                                className="pos-counseling-chips"
                                aria-label="Counselling label suggestions"
                              >
                                {suggestions.map((suggestion) => (
                                  <button
                                    className={
                                      item.labelInstruction?.includes(
                                        suggestion.instruction,
                                      )
                                        ? "active"
                                        : ""
                            ...
                            <textarea
                              value={item.labelInstruction ?? ""}
                              onChange={(event) =>
                                updateCart(item.rowId, {
                                  labelInstruction: event.target.value,
                                })
                              }
                              disabled={!canSell}
                              rows={3}
              ...
              {!cartRows.length && (
                <div className="empty-state">Add medicines to begin.</div>
              )}
            </div>
            ...
            <div className="pos-customer-panel">
              <div className="pos-customer-field">
                <label htmlFor="pos-customer-name">
                  <User2 size={15} /> Staff/patient
                </label>
                <input
                  id="pos-customer-name"
                  value={customerName}
                  onChange={(event) => setCustomerName(event.target.value)}
                  placeholder="Enter staff or patient name"
                  disabled={!canSell}
                />
                  ...
                  <div className="pos-patient-suggestions">
                    {nameMatches.map((profile) => (
                      ...
                      >
                        ...
                        <span>
                          {profile.phone} / {profile.sales.length} visit
                          {profile.sales.length === 1 ? "" : "s"}
                        </span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="pos-customer-field">
                ...
                <input
                  ...
                  placeholder="Enter staff or patient phone"
                  disabled={!canSell}
                />
              ...
              {selectedPatient && (
                <button
                  className="pos-patient-snapshot"
                  type="button"
                  onClick={() => selectPatientProfile(selectedPatient)}
                  disabled={!canSell}
                >
                  <strong>{selectedPatient.name}</strong>
                  <span>
                    {selectedPatient.phone} / {selectedPatient.sales.length}{" "}
                    previous visit
                    {selectedPatient.sales.length === 1 ? "" : "s"} / last seen{" "}
              ...
              <label className="full">
                <span>
                  <MessageSquare size={15} /> Follow-up message
                </span>
                <textarea
                  value={followUpMessage}
                  onChange={(event) => setFollowUpMessage(event.target.value)}
                  placeholder={
                    suggestedFollowUpMessage || "Optional follow-up message"
                  }
                  disabled={!canSell}
                  rows={3}
                />
              ...
              {suggestedFollowUpMessage && (
                <div className="pos-followup-suggestion">
                  ...
                  <button
                    type="button"
                    onClick={() => setFollowUpMessage(suggestedFollowUpMessage)}
                    disabled={!canSell}
                  >
                    Use suggested
                  </button>
            ...
            <div className="pos-action-row">
              ...
              <button
                className="complete"
                type="submit"
                disabled={!canDispense || !cartRows.length}
                title={
                  canDispense
                    ? "Dispense medicines and save unavailable items"
                    : "Only assigned pharmacists can dispense"
                }
              >
      ...
      {historyOpen && (
        ...
        >
          <section
            className="pos-modal-panel pos-history-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="pos-modal-header">
              ...
              <button
                ...
                aria-label="Close dispensing history"
              >
                <X size={18} />
              </button>
            </header>
            ...
            <div className="pos-sales-summary">
              <div>
                <span>Records</span>
                <strong>{number.format(filteredSales.length)}</strong>
              </div>
              <div>
                <span>Medicines</span>
                <strong>{number.format(filteredSalesItems)}</strong>
              </div>
              <div>
                <span>Patients</span>
                <strong>{number.format(filteredPatientCount)}</strong>
              </div>
              <div>
                <span>Cost captured</span>
                <strong>{money.format(filteredSalesTotal)}</strong>
              </div>
            </div>
            <div className="pos-modal-list">
              {filteredSales.map((sale) => (
                <article className="sale-history-item" key={sale.id}>
                  ...
                  <span>
                    {sale.customerName || "Staff member"}
                    {sale.customerPhone ? ` / ${sale.customerPhone}` : ""}
                  </span>
                  <ul>
                    {sale.items.map((item, index) => (
                      <li key={`${sale.id}-${index}`}>
                        {item.itemName || item.medicineId || item.productId}:{" "}
...
function PatientsView({
  ...
  const chronicPatients = profiles.filter((profile) => {
    const medicineSaleCount = profile.sales.reduce(
      (sum, sale) =>
        sum +
        sale.items.filter(
          (item) => item.itemType === "medicine" && item.daysSupply,
        ).length,
      0,
    );
  ...
  const careQueue = [
    {
      label: "Overdue refills",
      value: overdueRows.length,
      tone: "danger",
      detail: "Patients whose therapy window has already passed.",
    },
    {
      label: "Due this week",
      value: dueRows.filter((row) => row.daysUntilDue >= 0).length,
      tone: "warning",
      detail: "Patients to call or message before they miss a refill.",
    },
    {
      label: "Chronic relationship list",
      value: chronicPatients.length,
      tone: "good",
      detail: "Repeat patients worth retaining with consistent follow-up.",
    },
  ];
  const filteredProfiles = profiles.filter((profile) => {
    const text =
      `${profile.name} ${profile.phone} ${profile.sales.map((sale) => sale.reference).join(" ")}`.toLowerCase();
    return text.includes(query.toLowerCase());
  });
    ...
    <div className="page-grid patients-page">
      <section className="metric-grid">
        <Metric
          ...
          value={compactNumber(profiles.length)}
        />
        <Metric
          icon={Bell}
          label="Refills due soon"
          value={compactNumber(dueRows.length)}
          tone={dueRows.length ? "warning" : "good"}
        />
        <Metric
          icon={MessageSquare}
        ...
        <Metric
          icon={Phone}
          label="Phone-linked records"
          value={compactNumber(
            profiles.filter((profile) => profile.phone).length,
          )}
        />
      </section>
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Search by phone, name, or receipt reference across every
              Totalenergies clinic site.
            </p>
          </div>
          <span className="pill active">All Totalenergies sites</span>
        </div>
        <label className="patient-search">
          ...
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Search patient phone, name, or receipt..."
          />
        </label>
          ...
          <div className="patient-list">
            {filteredProfiles.map((profile) => (
              ...
              >
                <strong>{profile.name}</strong>
                <span>
                  {profile.phone || "No phone saved"} / {profile.sales.length}{" "}
                  visit{profile.sales.length === 1 ? "" : "s"}
                </span>
                <small>
                  Last seen {new Date(profile.lastVisit).toLocaleDateString()}{" "}
                  at{" "}
                  {getBranchName(
                    db,
                    profile.sales[0]?.branchId || activeBranch?.id || "main",
                  )}{" "}
                  / {money.format(profile.totalSpent)}
                </small>
              </button>
            ))}
            {!filteredProfiles.length && (
              <div className="empty-state">
                No patient records match this search yet.
              </div>
            )}
          </div>
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Generated from workspace-wide medicine dispensing records with
              therapy days captured.
            </p>
          </div>
          <span className="pill warning">
            {dueRows.length} due within 7 days
          </span>
        </div>
        <div className="patient-reminder-list">
          {dueRows.map((row) => {
            ...
            return (
              <article
                key={`${row.profileKey}-${row.item.medicineId}-${row.sale.id}`}
              >
                <div>
                  <strong>{row.patientName}</strong>
                  <span>
                    {row.medicineName} / refill {formatDate(row.refillDueAt)} /{" "}
                    {row.daysUntilDue < 0
                      ? `${Math.abs(row.daysUntilDue)} day${Math.abs(row.daysUntilDue) === 1 ? "" : "s"} overdue`
                      : `${row.daysUntilDue} day${row.daysUntilDue === 1 ? "" : "s"} left`}
                  </span>
                  <small>
                    {row.phone || "No phone saved"} / last visit{" "}
                    {getBranchName(db, row.sale.branchId)}
                  </small>
                </div>
                <footer>
                  <button
          ...
          {!dueRows.length && (
            <div className="empty-state">
              No refill reminders due in the next 7 days.
            </div>
          )}
        </div>
      </section>
...
function Adjustments({
  ...
  const isPositive =
    form.mode === "adjustment" || form.mode === "customer-return";
    ...
    <section className="content-section narrow">
      <div className="section-heading">
        <div>
          ...
          <p>
            Posting adjustments for {activeBranch.name}. All corrections require
            a reason.
          </p>
        </div>
      </div>
      {!canAdjust && (
        <div className="form-error">
          You only have view access in {activeBranch.name}, or your role cannot
          post adjustments.
        </div>
      )}
      <form className="form-grid" onSubmit={submit}>
        <label className="full">
          ...
          <select
            ...
            disabled={!canAdjust || !positiveRows.length}
          >
            <option value="">Select batch</option>
            {positiveRows.map((row) => (
              <option key={row.batch.id} value={row.batch.id}>
                {medicineOptionLabel(row.medicine)} / {row.batch.batchNumber} /
                Qty {medicineStockLabel(row.medicine, row.quantity)}
              </option>
            ))}
          </select>
        </label>
        <label>
          ...
          <select
            value={form.mode}
            onChange={(event) =>
              setForm({ ...form, mode: event.target.value as LedgerType })
            }
            disabled={!canAdjust}
          >
            <option value="write-off">Write-off</option>
            <option value="supplier-return">Supplier return</option>
            <option value="customer-return">Staff/patient return</option>
        ...
        {selected && (
          <div className="availability full">
            ...
            <span>
              ...
              expires {selected.batch.expiryDate}
            </span>
          </div>
        )}
        <div className="form-actions full">
...
function Reports({
    ...
    if (report === "movement") {
      return db.ledger
        ...
        .filter((entry) => {
          ...
          const semanticType =
            entry.reason === "Medicine dispensed"
              ? "pos"
              : entry.reason.startsWith("Internal requisition")
                ? "internal-transfer"
                : entry.type;
          const entryDate = entry.createdAt.slice(0, 10);
          return (
            (!movementStartDate || entryDate >= movementStartDate) &&
            (!movementEndDate || entryDate <= movementEndDate) &&
            (!movementType || semanticType === movementType) &&
            (!medicineFilter ||
              itemName.toLowerCase().includes(medicineFilter.toLowerCase())) &&
            (!genericFilter ||
              genericName
                .toLowerCase()
        ...
        .map((entry) => {
          ...
          const salesValue =
            entry.reason === "Medicine dispensed"
              ? Math.abs(entry.quantity) * unitPrice
              : 0;
          return {
  ...
  const movementQuantityTotal =
    report === "movement"
      ? rows.reduce((sum, row) => sum + Number(row.Quantity ?? 0), 0)
      : 0;
  const movementUnitsMoved =
    report === "movement"
      ? rows.reduce((sum, row) => sum + Math.abs(Number(row.Quantity ?? 0)), 0)
      : 0;
  const movementSalesTotal =
    report === "movement"
      ? rows.reduce(
          (sum, row) => sum + (Number(row["Dispensing Value"]) || 0),
          0,
        )
      : 0;
  const stockQuantityTotal =
    report === "stock"
      ? rows.reduce((sum, row) => sum + Number(row.Quantity ?? 0), 0)
      : 0;
      ...
      <div className="section-heading">
        ...
        <div className="button-row">
          <button
            className="ghost-button"
            type="button"
            onClick={() => exportCsv(`${report}-report.csv`, rows)}
      ...
      {report === "stock" && (
        <>
          ...
          <div className="report-summary">
            ...
            <span>
              pharmacy units on hand / {money.format(stockCostTotal)} value
            </span>
          </div>
        </>
      )}
      ...
      {report === "movement" && (
        <div className="report-summary">
          ...
          <span>
            ...
            {number.format(movementQuantityTotal)} / dispensing value{" "}
...
function QuestCoach({
  ...
  const branchCanSwitch =
    db.branches.filter((branch) => branch.active).length > 1;
  const complete = step
    ...
    >
      ...
      <div className="quest-status">
        ...
        <b className={complete ? "done" : ""}>
          {complete ? "Completed" : roleLabels[currentUser.role]}
        </b>
      </div>
...
const guideCards: GuideCard[] = [
  {
    ...
    summary:
      "Start every shift by confirming the clinic site at the top right of the file.",
    view: "dashboard",
    shot: "branch",
    steps: [
      "Open Dashboard.",
      "Use the site selector in the top bar.",
      "Choose the clinic site you are working from.",
      "Confirm cards and alerts refresh for that site.",
    ],
  },
  {
    id: "pos",
    ...
    summary:
      "Pharmacists can find in-stock medicines, add them to the prescription basket, track cost, and print reconciliation history.",
    view: "pos",
    shot: "pos",
    roles: ["pharmacist"],
    steps: [
      "Open Prescriptions.",
      "Search by medicine name, SKU, barcode, or use frequently used medicines.",
      "Add medicines to the basket and enter staff/patient details.",
      "Save a draft or dispense if you are assigned to that role.",
    ],
  },
  {
    id: "receive",
    ...
    summary:
      "Inventory staff receive medicines into the clinic pharmacy stock file.",
    view: "receive",
    shot: "receive",
    roles: ["inventory", "pharmacist", "admin"],
    steps: [
      "Open Receive.",
      "Search by medicine name, SKU, or barcode.",
      "Enter invoice, supplier, cost, value, quantity, batch, and expiry.",
      "Review calculated least-unit cost before posting.",
    ],
  },
  {
    id: "reports",
    ...
    summary:
      "Reports help beta testers check stock at hand, stock movement, dispensing value, suppliers, expiry, and reorder needs.",
    view: "reports",
    shot: "reports",
    steps: [
      "Open Reports.",
      "Choose Stock on hand or Movement ledger.",
      "Filter by date, type, brand, generic, or category, then export CSV or print.",
    ],
  },
  {
    id: "access",
    ...
    summary:
      "Super admins and site managers can review which employees can work inside a selected clinic site.",
    view: "branches",
    shot: "access",
    branchManagerOnly: true,
    steps: [
      "Open Sites.",
      "Select a clinic site card.",
      "Review Staff Access underneath the site list.",
  ...
  },
  {
    ...
    summary:
      "Super admins can confirm company identity, logo, default site, and operational thresholds.",
    view: "settings",
    shot: "settings",
    superAdminOnly: true,
    steps: [
      "Open Settings.",
      "Upload or remove the company logo.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Send operational notes without leaving the file.",
    ],
  },
];
    ...
    <section className="content-section guide-section">
      <div className="section-heading">
        <div>
          ...
          <p>
            Role-specific walkthroughs for beta testing the Totalenergies
            pharmacy file without guessing where to start.
          </p>
        </div>
...
function ChatView({
  ...
  function submit(event: FormEvent) {
    ...
    void executeAction(
      ...
      channel === "direct" ? "Direct message sent" : "Group message sent",
    );
    setBody("");
  }
    ...
    <section className="content-section chat-section">
      <div className="section-heading">
        <div>
          <h2>Messages</h2>
          <p>
            Send file updates to everyone or private notes to another employee.
          </p>
        </div>
      </div>
      <div className="chat-controls">
        <div className="report-mode-switch">
          <button
            className={channel === "group" ? "active" : ""}
            type="button"
        ...
        {messages.length ? (
          messages.map((message) => {
            ...
            const recipient = db.users.find(
              (item) => item.id === message.recipientUserId,
            );
            ...
            return (
              ...
              >
                <div>
                  <strong>
                    {mine ? "You" : (user?.name ?? "Team member")}
                  </strong>
                  <span>
                    {message.channel === "direct"
          ...
          <div className="empty-state">
            {channel === "direct" && !activeRecipient
              ? "Choose an employee to view or send direct messages."
              : "No messages yet."}
          </div>
        )}
      </div>
      <form className="chat-composer" onSubmit={submit}>
        ...
        <button
          className="primary-button"
          type="submit"
          disabled={!body.trim() || (channel === "direct" && !recipientUserId)}
        >
          <Send size={17} />
          Send
        </button>
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              New registrations stay pending until an admin assigns a role and
              ...
              {db.users.map((user) => (
                <tr key={user.id}>
                  <td>
                    ...
                    <span>
                      {user.email}
                      {user.id === primaryAdminId ? " / Permanent admin" : ""}
                    </span>
                  </td>
                  <td>{user.phone || "-"}</td>
                  <td>
                  ...
                  <td>
                    <select
                      ...
                      disabled={
                        user.id === currentUser.id || user.id === primaryAdminId
                      }
                    >
                  ...
                  <td>
                    <span>
                      {isSuperAdmin(db, user)
                        ...
                        : db.branches
                            .filter((branch) =>
                              hasActiveBranchAssignment(user, branch.id),
                            )
                            .map((branch) => {
                              const expiresAt = getBranchAccessExpiry(
                                user,
                                branch.id,
                              );
                              return `${branch.name}${expiresAt ? ` until ${expiresAt}` : ""}`;
                            })
                            .join(", ") || "View only everywhere"}
                    </span>
                  </td>
                  <td>
                    <span className={`pill ${user.status}`}>
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Password resets, new device sign-ins, panic actions, and email
              delivery issues are visible here for admin oversight.
            </p>
          </div>
        </div>
        <div className="alert-list">
          {securityEvents.length ? (
            securityEvents.map((event) => {
              ...
              return (
                <article
                  className={`alert-item ${event.severity === "critical" ? "danger" : event.severity === "warning" ? "warning" : "info"}`}
                  key={event.id}
                >
                  {event.type === "panic-triggered" ? (
                    <ShieldCheck size={19} />
                  ) : (
                    <Lock size={19} />
                  )}
                  <div>
                    ...
                    <span>
                      {user?.name ?? event.email} / {event.email} /{" "}
...
function BranchesView({
  ...
  const selectedBranch =
    db.branches.find((branch) => branch.id === activeBranchId) ??
    db.branches[0];
  const primaryAdminId = getPrimaryAdminId(db);
  const superAdmin = isSuperAdmin(db, currentUser);
  const canManageSelected = selectedBranch
    ? canManageBranch(db, currentUser, selectedBranch.id)
    : false;
  const assignableUsers = useMemo(
    () =>
      db.users.filter(
        (user) => user.status === "active" && user.id !== primaryAdminId,
      ),
    [db.users, primaryAdminId],
  );
  ...
  const [accessExpiryByUser, setAccessExpiryByUser] = useState<
    Record<string, string>
  >({});
  const currentUserHasSelectedAccess = selectedBranch
    ? hasActiveBranchAssignment(currentUser, selectedBranch.id)
    : false;
  const currentUserPendingAccessRequest = selectedBranch
    ? db.branchAccessRequests.find(
        (request) =>
          request.userId === currentUser.id &&
          request.branchId === selectedBranch.id &&
          request.status === "pending",
      )
    : undefined;
  ...
  function updateBranchAccess(user: User, canAccess: boolean) {
    ...
    const expiresAt =
      accessExpiryByUser[draftKey] ??
      getBranchAccessExpiry(user, selectedBranch.id);
    void executeAction(
      "updateBranchAccess",
      {
        userId: user.id,
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              The account is the company dashboard; branches hold stock and
            ...
            >
              ...
              <div>
                ...
                <span>
                  ...
                  {getUserBranchStatus(db, currentUser, branch.id)}
                </span>
                <span>
                  {db.users.find((user) => user.id === branch.managerUserId)
                    ?.name ||
                    branch.managerName ||
                    "No manager"}{" "}
              ...
              <button
                ...
                type="button"
                onClick={() => edit(branch)}
                disabled={!canManageBranch(db, currentUser, branch.id)}
                title="Edit branch"
              >
                <ClipboardList size={16} />
              </button>
          ...
          <div className="branch-access-panel">
            <div className="section-heading">
              <div>
                ...
                <p>
                  Super admin can assign managers and override transfers. Branch
                  managers can grant access to free staff for their own branch.
                </p>
              </div>
              <span
                className={canManageSelected ? "pill active" : "pill muted"}
              >
                {canManageSelected ? "Can authorize" : "View only"}
              </span>
            </div>
            {!canManageSelected &&
              !currentUserHasSelectedAccess &&
              currentUser.id !== primaryAdminId && (
                <div className="access-request-box">
                  ...
                  <button
                    ...
                    disabled={Boolean(currentUserPendingAccessRequest)}
                  >
                    <Send size={16} />
                    {currentUserPendingAccessRequest
                      ? "Request sent"
                      : "Request access"}
                  </button>
                </div>
            ...
            <div className="access-list">
              {assignableUsers.map((user) => {
                const hasAccess = hasActiveBranchAssignment(
                  user,
                  selectedBranch.id,
                );
                const isManager =
                  user.managedBranchIds.includes(selectedBranch.id) &&
                  hasActiveBranchAssignment(user, selectedBranch.id);
                const otherAssignedBranch = getOtherAssignedBranch(
                  db,
                  user,
                  selectedBranch.id,
                );
                const waitingForRelease = Boolean(
                  otherAssignedBranch && !hasAccess && !superAdmin,
                );
                return (
                  <article className="access-row" key={user.id}>
                    <input
                      ...
                      checked={hasAccess}
                      onChange={(event) =>
                        updateBranchAccess(user, event.target.checked)
                      }
                      disabled={
                        !canManageSelected || isManager || waitingForRelease
                      }
                    />
                    <span>
                      <strong>{user.name}</strong>
                    ...
                    <div className="access-row-controls">
                      ...
                      {canManageSelected && !isManager && (
                        <label className="access-expiry">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setAccessExpiryByUser((current) => ({
                                ...current,
                                [`${selectedBranch.id}:${user.id}`]:
                                  event.target.value,
                              }))
                            }
                            disabled={waitingForRelease}
                          />
                        </label>
                      )}
                      {canManageSelected && hasAccess && !isManager && (
                        <button
                          className="icon-button"
                          type="button"
                          onClick={() => updateBranchAccess(user, true)}
                          title="Save access timeframe"
                        >
      ...
      <section className="content-section">
        <div className="section-heading">
          <div>
            ...
            <p>
              Managers can maintain their branch. Only admins can create
              branches or assign branch managers.
            </p>
          </div>
        </div>
        {!canManageSelected && form.id && (
          <div className="form-error">
            You can view {form.name}, but only its manager or an admin can edit
            it.
          </div>
        )}
        <form className="form-grid" onSubmit={submit}>
          <label className="full">
            ...
            <input
              ...
              disabled={form.id ? !canManageSelected : !superAdmin}
            />
          </label>
          <label>
            Code
            ...
            <select
              ...
              onChange={(event) =>
                setForm({
                  ...
                  managerName:
                    db.users.find((user) => user.id === event.target.value)
          ...
          <label className="full">
            ...
            <input
              ...
              onChange={(event) =>
                setForm({ ...form, managerName: event.target.value })
              }
              placeholder="Enter manager or contact name"
              disabled={form.id ? !canManageSelected : !superAdmin}
            />
          </label>
          <label className="full">
          ...
          <label className="checkbox-row full">
            <input
              ...
              disabled={!superAdmin || form.id === "main"}
            />{" "}
            Active branch
          </label>
          <div className="form-actions full">
      ...
      <form className="form-grid" onSubmit={submit}>
        <div className="workspace-logo-editor full">
          ...
          <div>
            ...
            <span>
              This replaces the default capsule mark inside the Totalenergies
              pharmacy file.
            </span>
          </div>
          <label className="file-button">
        ...
        <label>
          ...
          <input
            type="number"
            min="1"
            value={numberInputValue(form.nearExpiryDays)}
            onChange={(event) =>
              setForm({ ...form, nearExpiryDays: Number(event.target.value) })
            }
            disabled={!canAdmin}
          />
        </label>
        <label>
          ...
          <input
            type="number"
            min="0"
            value={numberInputValue(form.approvalThreshold)}
            onChange={(event) =>
              setForm({
                ...form,
        ...
        <div className="counseling-settings full">
          <div className="pricing-settings-head">
            <div>
              ...
              <p>
                Suggested chips appear in the prescription basket and print on
                medicine labels only. Reconciliation history stays clean.
              </p>
            </div>
          </div>
          <div className="pricing-settings-grid">
            <label>
              ...
              <input
                value={form.defaultFollowUpLabel ?? "Follow-up"}
                onChange={(event) =>
                  setForm({ ...form, defaultFollowUpLabel: event.target.value })
                }
                disabled={!canAdmin}
              />
            </label>
            <label className="full">
              ...
              <textarea
                value={form.defaultFollowUpMessage ?? ""}
                onChange={(event) =>
                  setForm({
                    ...form,
                    defaultFollowUpMessage: event.target.value,
                  })
                }
                disabled={!canAdmin}
                rows={3}
                placeholder="Please contact the clinic pharmacy if symptoms persist or side effects occur."
              />
            </label>
          </div>
          <div className="settings-chip-preview">
            {Object.entries(textToDosageFormRules(dosageFormLabelText))
              .slice(0, 8)
              .map(([key]) => (
                <span key={key}>{splitRuleKeys(key)[0] || key}</span>
              ))}
          </div>
          <div className="pricing-rule-editors">
            <label>
              ...
              <textarea
                value={dosageFormLabelText}
                onChange={(event) => setDosageFormLabelText(event.target.value)}
                disabled={!canAdmin}
                rows={8}
                placeholder={
                  "tablet, tablets = Swallow with water.\noral liquid, syrup = Shake well before use."
                }
              />
            </label>
            <label>
              ...
              <textarea
                value={medicineLabelText}
                onChange={(event) => setMedicineLabelText(event.target.value)}
                disabled={!canAdmin}
                rows={8}
                placeholder={
                  "metronidazole | Avoid alcohol | Do not take alcohol during treatment.\ncataflam, diclofenac | Take with food | Take with or after food."
                }
              />
            </label>
          </div>
        ...
        <div className="pricing-settings full">
          <div className="pricing-settings-head">
            <div>
              ...
              <p>
                Calculate prices from final least-unit cost, markup, and
                rounding. Manual selling price is locked while this is on,
                except admin override with audit reason.
              </p>
            </div>
            <label className="switch-row">
              <input
                ...
                onChange={(event) =>
                  setForm({ ...form, autoPricingEnabled: event.target.checked })
                }
                disabled={!canAdmin}
              />
          ...
          <div className="pricing-settings-grid">
            <label>
              ...
              <input
                type="number"
                min="0"
                value={numberInputValue(form.globalMarkupPercent ?? 30)}
                onChange={(event) =>
                  setForm({
                    ...form,
                    globalMarkupPercent: Number(event.target.value),
                  })
                }
                disabled={!canAdmin}
              />
            ...
            <label>
              Round prices
              <select
                value={form.pricingRoundingRule ?? 10}
                onChange={(event) =>
                  setForm({
                    ...form,
                    pricingRoundingRule: Number(
                      event.target.value,
                    ) as PricingRoundingRule,
                  })
                }
                disabled={!canAdmin}
              >
                {pricingRoundingRules.map((rule) => (
                  <option key={rule} value={rule}>
                    {rule ? `Nearest ${money.format(rule)}` : "No rounding"}
                  </option>
                ))}
              </select>
            </label>
            <label>
              ...
              <input
                ...
                min="0"
                max="100"
                value={numberInputValue(form.managerDiscountLimitPercent ?? 10)}
                onChange={(event) =>
                  setForm({
                    ...form,
                    managerDiscountLimitPercent: Number(event.target.value),
                  })
                }
                disabled={!canAdmin}
              />
            ...
            <label>
              Unusual markup warning %
              <input
                type="number"
                min="0"
                value={numberInputValue(form.unusualMarkupPercent ?? 80)}
                onChange={(event) =>
                  setForm({
                    ...form,
                    unusualMarkupPercent: Number(event.target.value),
                  })
                }
                disabled={!canAdmin}
              />
            ...
            <label>
              ...
              <input
                type="number"
                min="0"
                value={numberInputValue(form.costChangeWarningPercent ?? 30)}
                onChange={(event) =>
                  setForm({
                    ...form,
                    costChangeWarningPercent: Number(event.target.value),
                  })
                }
                disabled={!canAdmin}
              />

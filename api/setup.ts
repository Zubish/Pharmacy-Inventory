...
export default async function handler(
  ...
  try {
    ...
    const pharmacyName =
      body.pharmacyName?.trim() || TOTALENERGIES_ACCOUNT_NAME;
    if (
    ...
    if (root.tenants.some((tenant) => tenant.slug === companySlug)) {
      fail(
        res,
        409,
        "Totalenergies Pharmacy Inventory has already been set up",
      );
      return;
    }
    ...
    addAudit(
      db,
      adminId,
      "Completed first-run Totalenergies pharmacy setup",
      "system",
      "setup",
    );
    ...
    fail(
      ...
      error instanceof Error
        ? error.message
        : "Unable to set up Totalenergies pharmacy file",
    );
  }
}

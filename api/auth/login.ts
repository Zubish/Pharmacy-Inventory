...
export default async function handler(
  ...
  try {
    ...
    const user = db.users.find(
      (item) => item.email === body.email?.trim().toLowerCase(),
    );
    ...
    if (knownDevice) {
      ...
      try {
        await sendSecurityEmail(
          user.email,
          "New Totalenergies Pharmacy Inventory sign-in detected",
          `A new sign-in to your Totalenergies Pharmacy Inventory account was detected.\n\nDevice: ${userAgent || "Unknown"}\nIP: ${ipAddress || "Unknown"}\n\nIf this was not you, sign in and use Secure my account.`,
        );
      } catch (error) {
        addSecurityEvent(db, {
          userId: user.id,
          ...
          detail:
            error instanceof Error
              ? error.message
              : "Unable to send new-device email alert.",
          ipAddress,
          userAgent,
        });
      }

  ...
  try {
    ...
    if (existing) {
      if (new Date(existing.expiresAt).getTime() <= Date.now()) {
        ...
        fail(
          res,
          409,
          "A password reset code is already active. Check your email or wait for it to expire.",
        );
        return;
      }
    }
    ...
    try {
      const sent = await sendSecurityEmail(
        email,
        "Your Totalenergies Pharmacy Inventory password reset code",
        `Use this Totalenergies Pharmacy Inventory password reset code within 15 minutes:\n\n${code}\n\nIf you did not request this, secure your account or contact your account admin.`,
      );
      emailSent = sent.sent;
    ...
    addSecurityEvent(db, {
      ...
      severity: "warning",
      detail: emailSent
        ? "A password reset code was sent to the user email."
        : "A password reset was requested, but email is not configured yet.",
      ipAddress: getRequestIp(req),
      userAgent: getRequestUserAgent(req),

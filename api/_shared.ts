...
export function getCompanySlugFromRequest(req: HandlerRequest) {
  const header =
    req.headers["x-totalenergies-company"] ?? req.headers["x-rxledger-company"];
  const raw = Array.isArray(header) ? header[0] : header;
  return (
    normalizeCompanySlug(raw || TOTALENERGIES_COMPANY_SLUG) ||
...
export function normalizeDatabase(raw: Partial<Database>): Database {
  ...
  const companySlug =
    normalizeCompanySlug(rawSettings.companySlug || accountName) ||
    TOTALENERGIES_COMPANY_SLUG;
  const isTotalenergiesFile =
    companySlug === TOTALENERGIES_COMPANY_SLUG ||
    accountName.toLowerCase().includes("totalenergies");
  ...
  return {
    ...
    settings: {
      ...empty.settings,
      ...rawSettings,
      softwareName: isTotalenergiesFile
        ? "Totalenergies Pharmacy Inventory"
        : rawSettings.softwareName || "Totalenergies Pharmacy Inventory",
      accountName,
